...
이 중 가장 큰 비중을 차지하는 것은 **도구 결과**와 **대화 히스토리**다. 두 항목만 줄여도 전체 토큰 소비의 60~70%를 절감할 수 있다.
...
2000줄짜리 파일 전체를 읽으면 약 50,000 토큰이 소비될 수 있다. 같은 파일의 특정 구간 20줄만 읽으면 약 500 토큰이다. 지시를 구체적으로 하는 것만으로 100배 차이가 난다.
