...
Redis 설치·기동은 [08-5](08-5-redis-why.md)에서 끝냈다. 여기서는 설치된 Redis로 바로 실습한다.
...
팀이 키 이름을 통일해 두면 충돌과 혼선을 막을 수 있다. 표준 스키마는 다음과 같다.
...
하트비트는 에이전트가 살아있다고 알리는 신호다. TTL을 함께 걸어 두면 갑자기 죽어도 자동으로 "응답 없음"으로 바뀐다.
