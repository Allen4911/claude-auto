...
각 역할은 작업을 독립적으로 수행하지만, 결과물은 다음 역할로 이관된다. 이관이 원활하게 이뤄지려면 **산출물 경로와 형식**이 사전에 약속되어야 한다.
...
위 표처럼 서로 독립적인 조사 단계는 병렬로, 앞 결과에 의존하는 설계 → 구현 → 리뷰는 순차로 배분된다.
...
> 💡 **복합 지시 처리 원칙**: 지시에 여러 역할이 필요한 키워드가 섞여 있으면 Phase로 분해한다. "분석하고 구현하고 리뷰해줘" = Phase1(지훈·조사) + Phase2(서연·구현) + Phase3(태양·리뷰)로 자동 변환된다.
...
> 💡 **충돌 예방의 핵심**: 팀장이 배분할 때 "이 작업은 민준 담당, 저 작업은 서연 담당"처럼 파일 또는 영역 단위로 명시하면 충돌이 크게 줄어든다. 애매한 경우엔 팀원이 판단하는 것보다 팀장에게 물어보는 것이 낫다는 규칙을 CLAUDE.md에 추가하는 것도 효과적이다.
