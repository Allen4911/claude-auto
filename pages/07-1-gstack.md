...
> 🔀 **네이티브 vs 팀**: gstack 스킬을 네이티브 단일 세션에서 쓸 때는 `/review`, `/cso` 같은 슬래시 커맨드를 개발자가 직접 호출하는 즉시 호출 방식입니다. TMUX 팀 환경에서는 같은 스킬을 각 팀원의 CLAUDE.md에 등록해 두면, 지시가 오는 순간 역할에 맞는 워크플로우가 자동으로 실행되는 자동화 방식으로 전환됩니다. 스킬 자체는 동일하지만 호출 주체(사람 vs 팀 아키텍처)가 달라집니다. → [3-4. Skills 작성으로 작업 표준화](03-4-skills-authoring.md)
