...
팀 에이전트를 효과적으로 운용하려면 **지시가 어떻게 흘러가는지**를 명확히 설계해야 합니다. 명확한 흐름이 없으면 팀원 간 작업이 중복되거나, 지시가 누락되거나, 결과가 사용자에게 돌아오지 않습니다.

이 책에서 채택한 지시 흐름은 **단일 진입점 패턴**입니다.
...
사용자가 팀원에게 직접 지시를 내리는 **직접 지시 패턴**도 가능하지만, 다음과 같은 이유로 단일 진입점을 권장합니다.
...
지시 하나는 아래 5개 Phase를 순서대로 거칩니다. 수신에서 보고까지 한 줄기로 이어집니다.

다섯 Phase는 ①지시 수신 → ②분석 및 분해 → ③팀원 배분 → ④진행 관리 → ⑤결과 보고 순서로 이어집니다. 각 단계는 아래에서 차례로 설명합니다.

### Phase 1: 지시 수신

사용자의 지시가 팀장에게 도달하는 경로는 세 가지입니다.
...
분해한 작업마다 **담당자·산출물 경로·완료 기준**을 함께 정해 두면 팀원이 무엇을 어디까지 해야 하는지 명확해집니다. "대충 분석해줘"보다 "/tmp/payment-analysis.md에 인터페이스와 의존성을 정리해줘"처럼 파일 경로와 항목을 명시하는 것이 훨씬 효과적입니다.

<hr>

### Phase 3: 팀원 배분

분해된 작업을 `tmux send-keys` 명령으로 각 팀원에게 전달합니다.
...
배분 직후 팀장은 팀원 파인을 확인해 지시가 수신되었는지 점검합니다.
...
아무 반응이 없으면 파인이 블로킹 상태일 수 있습니다. `tmux send-keys -t team:0.2 "" Enter`로 빈 Enter를 보내 세션을 깨운 뒤 재전달합니다.

<hr>

### Phase 4: 진행 관리

팀장은 각 팀원의 작업 진행 상황을 모니터링합니다.
...
화면에 오류 메시지나 긴 침묵이 보이면 팀원이 블로킹된 것일 수 있습니다. 이 경우 현황 보고를 요청합니다.

```bash
tmux send-keys -t team:0.2 "현재 작업 진행 상황을 한 문장으로 보고해줘" Enter
```

5분 이상 응답이 없거나 화면이 바뀌지 않으면 파인 재기동을 검토합니다. 팀장은 진행 관리 도중 사용자에게 **중간 보고**를 보내 대기 상황을 알려 주면 좋습니다.

<hr>

### Phase 5: 결과 보고

모든 작업이 완료되면 팀장이 결과를 종합하여 사용자에게 보고합니다.
...
보고는 항상 **무엇이 끝났고 다음에 뭘 할지**를 함께 담으면 좋습니다. 사용자가 후속 지시를 내릴 때 맥락이 이어지기 때문입니다.

<hr>

## 따라하기: 단순 조사 작업 전체 흐름

"현재 프로젝트의 주요 파일 목록을 정리해줘"라는 지시가 들어왔을 때의 전체 흐름 실습입니다.
...
이 흐름이 자동으로 작동하려면 CLAUDE.md에 명확한 규칙을 정의해야 합니다.
...
팀을 처음 구성할 때 아래 최소 구성을 복사해서 팀장 CLAUDE.md로 저장합니다.
...
작업 특성에 따라 병렬 또는 순차로 배분합니다.

핵심 판단 기준은 작업 사이의 의존성 유무입니다.
...
파인이 완전히 멈춘 경우엔 `reboot-pane.sh`로 재기동한 뒤, 역할 정체성을 재주입하고 이전 작업 맥락을 요약하여 재지시합니다. 재기동은 컨텍스트가 초기화되므로 "이전에 요청한 작업은 X였고, 결과를 /tmp/Y.md에 저장하면 된다"처럼 맥락을 함께 전달해야 합니다.
