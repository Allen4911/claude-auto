...
Bot Mode는 Claude Code가 **메시지 접두사를 기반으로 지시를 수신하고, 작업 완료 후 결과를 자동으로 보고하는** 동작 방식입니다. Remote-Control을 통해 모바일이나 웹에서 메시지를 보내면, 팀장(Pane 0)이 이를 수신하고 팀원에게 분배한 뒤 결과를 돌려보냅니다.
...
Bot Mode는 아래 3단계로 한 바퀴 돕니다. 사용자의 한 번 지시가 팀장과 팀원을 거쳐 다시 사용자에게 답이 돌아옵니다.

세 단계는 ①Remote-Control로 지시 수신 → ②팀원에게 작업 배분 → ③결과 종합 및 보고이며, 아래에서 차례로 살펴봅니다.

### 1단계: Remote-Control로 지시 수신

모바일 앱 또는 claude.ai/code에서 메시지를 전송하면 Remote-Control을 통해 로컬 세션(Pane 0)에 실시간으로 전달됩니다.
...
팀장의 CLAUDE.md에 메시지 접두사 처리 규칙이 정의되어 있어야 합니다. 예를 들어 `[채널명:12345]` 접두사가 붙은 메시지는 Bot Mode 지시로 인식하고, 응답도 같은 채널로 보내야 한다는 규칙입니다.
...
Bot Mode를 시작하려면 팀장 파인에서 Remote-Control 플래그를 함께 써서 Claude를 실행합니다.
...
세션 이름을 명시하면 claude.ai/code 목록에서 해당 세션을 이름으로 찾을 수 있습니다.

<hr>

### 2단계: 팀원에게 작업 배분

팀장은 `tmux send-keys`로 각 팀원에게 지시를 전달합니다.
...
배분 직후 팀장은 각 팀원 파인 화면을 간단히 확인해 지시가 전달되었는지 점검합니다. 지시가 도달했다면 Claude가 처리를 시작하는 메시지가 표시됩니다.
...
팀원이 작업 완료 후 팀장에게 보고하면, 팀장이 결과를 종합하여 Remote-Control을 통해 사용자에게 응답합니다.
...
팀장은 팀원들의 보고를 그대로 옮기지 않고 **추려서 요약해** 사용자가 한눈에 알아보게 다듬습니다. 기술적 세부사항은 줄이고 "무엇이 됐고, 다음엔 뭘 해야 하는가"로 압축하는 것이 좋습니다.

> 💡 **보고 품질 기준** — 좋은 보고에는 끝낸 일, 결정이 필요한 일, 다음에 할 일이 들어간다. 이 세 가지가 있으면 사용자가 다음 지시를 내리기 쉬워진다.
...
팀장의 행동 원칙을 CLAUDE.md에 명시하면 일관된 팀 운용이 가능합니다.
...
Bot Mode에서 응답을 보낼 채널 규칙도 CLAUDE.md에 함께 포함해야 합니다.
...
이 규칙이 CLAUDE.md에 있으면 팀장이 어느 채널에서 지시가 왔는지 알아서 같은 채널로 답합니다. 채널 규칙이 없으면 응답 방향을 매번 직접 정해야 해 헷갈립니다.

<hr>

## 팀원 간 비동기 보고 패턴

팀원이 장시간 작업 후 완료를 알릴 때는 팀장 파인에 직접 메시지를 보냅니다.
...
팀장은 모든 팀원의 보고를 취합한 뒤 사용자에게 한 번에 결과를 전달합니다.

> 💡 **비동기 패턴의 장점** — 팀원들은 자기 속도대로 작업하고 완료되면 보고한다. 팀장은 일일이 체크하는 대신 보고가 오면 그때 수합한다. 이렇게 굴러가려면 팀원 CLAUDE.md에 "작업 완료 시 반드시 팀장(Pane 0)에게 보고"라는 규칙을 넣어야 한다.

팀원 CLAUDE.md에는 아래 규칙을 추가합니다.
...
"서버 상태 확인해줘" 메시지가 모바일에서 도착한 경우의 전체 흐름입니다.
