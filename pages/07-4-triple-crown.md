...
코드를 한 줄도 작성하기 전에, **만들 가치가 있는지**부터 확인한다.

### /cso — CEO 관점 검토

```
사용자: "실시간 협업 에디터를 만들고 싶어."

/cso 실행 후 Claude의 분석:

  비즈니스 가치: ★★★★☆
  - 기존 Google Docs, Notion 대비 차별점 필요
  - 실시간 동기화는 기술적 복잡도 높음
  - MVP 범위: 텍스트 편집 + 커서 공유로 축소 권장

  우선순위 판단:
  - P0: 텍스트 동기화 엔진 (CRDT)
  - P1: 커서 위치 공유
  - P2: 댓글/코멘트 기능
  - P3: 버전 히스토리 → 2차 마일스톤으로 이연
```

/cso는 기능의 비즈니스 타당성을 검토하고, 범위를 적절히 조정한다. "다 만들어"가 아니라 "이것부터 먼저" 순서를 정하는 것이다.

### /autoplan — 개발 계획 자동 생성

/cso에서 범위가 정해지면 /autoplan으로 실행 계획을 수립한다.
...
│                                                         │
│  Phase 1: 전략 (gstack)                                 │
│    쭌(팀장): /cso 결과 분석, 범위 결정                   │
│    민준(PM): /autoplan 계획 검토, 아키텍처 확인          │
│                                                         │
│  Phase 2: 구조화 (GSD)                                  │
│    민준(PM): /gsd:plan-phase 주도                        │
│    지훈(리서쳐): 기술 조사 → 가정 검증                   │
│                                                         │
│  Phase 3: 구현 (GSD + Superpowers)                      │
│    서연(개발자): /gsd:execute-phase + TDD                │
│    수아(디자이너): UI 컴포넌트 + 디자인 시스템           │
│    지훈(리서쳐): 기술 문제 조사 지원                     │
│                                                         │
│  Phase 4: 검증 (GSD + gstack)                           │
│    태양(리뷰어): /review + /qa 실행                      │
│    민준(PM): /gsd:validate-phase 확인                    │
│                                                         │
│  Phase 5: 완료 (gstack + GSD)                           │
│    쭌(팀장): /ship 승인, 사용자에게 보고                 │
│    민준(PM): /gsd:complete-milestone 처리                │
│                                                         │
└─────────────────────────────────────────────────────────┘
```
...
echo "=== Phase 1: 전략 수립 ==="
# 팀장이 /cso, /autoplan 실행 (Pane 0에서 직접)
echo "팀장: /cso와 /autoplan으로 전략 수립 완료"
...
Claude: (바로 코드 작성 시작)
  → 30분 후: "완성했습니다"
  → 문제: 오프라인 모드 미고려, 테스트 없음, 3명 이상 동시 편집 불가
  → 수정 요청 → 재구현 → 또 수정 → ...
```
...
/cso: "MVP 범위를 텍스트 동기화 + 커서 공유로 축소합니다"
/autoplan: "3개 Phase, 12개 태스크로 분해했습니다"
/gsd:plan-phase: "Phase 1의 5개 태스크와 완료 기준입니다"
/gsd:execute-phase + TDD: "테스트 47건 통과, 코드 구현 완료"
/review + /qa: "에러 핸들링 1건 수정, 성능 임계값 1건 조정"
/ship: "PR #15 생성, 배포 준비 완료"
