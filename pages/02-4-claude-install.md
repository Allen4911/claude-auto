...
![npm 설치 → claude --version 확인 → 버전 출력으로 이어지는 2단계 설치 검증 흐름도](../assets/02-4-claude-install-install-verify.png)
...
![폴더 신뢰 → 이용 약관 동의 → 로그인 방식 선택 → 브라우저 OAuth로 이어지는 인증 3단계 흐름](../assets/02-4-claude-install-auth-3steps.png)
