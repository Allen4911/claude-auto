...
사람 팀에서도 이런 문제가 발생하지만, AI 에이전트는 더 빠르게 더 많은 파일을 수정합니다. 충돌은 설계 단계에서 미리 막아야 합니다.
...
정리하면 충돌은 두 에이전트가 같은 파일을 동시에 수정할 때(파일 충돌), 또는 한쪽 변경이 다른 쪽이 의존하는 인터페이스를 깨뜨릴 때(로직 충돌) 발생합니다.
...
앞 단계가 끝난 뒤 다음 단계가 시작되므로, 두 에이전트가 같은 파일을 동시에 건드릴 일이 없습니다.
...
여섯 단계는 충돌 파일 열기 → 마커 확인 → 올바른 버전 선택 → 마커 제거 → 스테이징 → 커밋 순서입니다.
...
민준이 판단하여 지시를 내리면 서연이 해결합니다. 에이전트가 충돌을 혼자 해결하면 위험합니다.
...
실제 팀에서 자주 발생하는 세 가지 충돌 유형(동시 imports 수정, 타입 정의 변경, 테스트 파일 덮어쓰기)과 각 해결 방법은 **[09-7절 실전 충돌 사례와 해결](09-7-conflict-cases.md)** 을 참조하십시오.
...
작업 전·후로 확인하면 대부분의 충돌을 미리 걸러낼 수 있습니다.
...
이 설정을 저장소에 커밋하면 모든 에이전트가 동일한 규칙으로 코드를 작성합니다. 스타일 차이에서 생기던 diff가 사라져 리뷰 부담도 줄어듭니다.
...
이렇게 pre-commit 훅이 커밋 직전 ESLint·Prettier를 자동 적용하므로, 에이전트별 스타일 차이가 커밋에 남지 않습니다.
...
동결 공지, 리팩토링, 재동기화 순으로 진행하면 대규모 변경 중에도 충돌을 막을 수 있습니다.
