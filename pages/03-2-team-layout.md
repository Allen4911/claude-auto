...
> 💡 **모델을 왜 다르게 쓸까요?** Opus는 더 똑똑하지만 느리고 비싸고, Sonnet은 빠르고 저렴합니다. 깊은 판단이 필요한 설계(민준)에는 Opus, 정해진 작업을 빠르게 처리하는 역할에는 Sonnet을 배정해 "성능과 비용의 균형"을 맞추는 것입니다.
...
실제로 `tmux list-panes -t team`을 실행하면 위 표의 여섯 칸이 `0:`, `1:`…처럼 번호와 함께 한 줄씩 찍혀 나옵니다. 화면상 위치(좌·우, 위·아래)와 무관하게 **번호는 만들어진 순서대로 0번부터** 매겨지므로, 어떤 명령을 어느 파인에 보낼지는 이 번호로 지목합니다. 즉 표의 "Pane" 열이 곧 명령을 보낼 주소인 셈입니다.
...
위 스크립트가 하는 일을 순서로 풀면 다섯 단계입니다 — ① 세션을 새로 만들고 → ② 화면을 파인 6칸으로 쪼갠 뒤 → ③ `main-vertical` 레이아웃으로 정렬하며 팀장 너비를 키우고 → ④ 파인 제목 표시를 켠 다음 → ⑤ 각 파인에 이름표를 붙입니다. 한 번 실행하면 이 다섯 단계가 자동으로 이어져, 표에서 본 6인 팀 배치가 그대로 화면에 완성됩니다.
...
두 레이아웃은 쓰임이 갈립니다. `main-vertical`은 왼쪽 한 칸(팀장)에 시선이 쏠리는 구조라, 지시가 한 곳을 거쳐 흐르는 우리 팀 운영에 맞습니다. 반면 `even-horizontal`은 모든 파인을 같은 크기로 늘어놓아, 누가 더 중요할 것 없이 전원을 동등하게 들여다볼 때 유리합니다. 정리하면 **지휘 중심이면 main-vertical, 균등 감시면 even-horizontal**입니다.
