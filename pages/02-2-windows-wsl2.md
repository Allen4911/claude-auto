...
크게 보면 흐름은 네 묶음입니다 — **WSL2를 켜고 Ubuntu를 올린 뒤**(1~4단계), **시스템을 최신화하고 기본 유틸리티를 깔고**(5~6단계), **Node.js·Git·TMUX 개발 도구를 설치**(7~9단계)한 다음, 마지막으로 **Claude Code를 설치**(10단계)합니다. 세부 단계가 아래에 1번부터 차례로 나오니, 지금은 "WSL2 켜기 → 도구 준비 → Claude 설치"라는 큰 줄기만 잡으면 됩니다.
...
핵심은 이 연결이 양방향이라는 점입니다. Windows 탐색기에서 만든 파일을 Ubuntu 터미널에서 그대로 열 수 있고, 반대로 Ubuntu에서 작업한 결과도 Windows 탐색기에서 바로 확인됩니다. 한쪽에서 복사해 옮길 필요 없이 같은 파일을 두 환경이 공유하는 셈이라, 코드는 Ubuntu에서 다루고 결과물은 익숙한 Windows 앱으로 여는 혼합 작업이 자연스럽습니다.
