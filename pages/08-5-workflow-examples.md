...
지금까지 팀 구성, Remote-Control, Bot Mode, 업무 분담을 각각 다루었습니다. 이 장에서는 이 모든 걸 **하나의 자동 워크플로우**로 묶는 실전 예시를 다룹니다.

자동화 워크플로우의 핵심은 **사용자가 한 마디만 하면 여러 단계 작업이 알아서 펼쳐진다**는 것입니다.
...
![사용자의 한 마디 지시가 조사→설계→구현→리뷰→보고로 자동 전개되는 워크플로우 개념도](../assets/08-5-workflow-auto.png)

> 💡 **워크플로우를 미리 만들어두는 이유** 매번 "지훈아 조사해줘, 수아야 설계해줘"를 일일이 입력하면 사용자가 직접 팀을 지휘하는 셈입니다. 워크플로우 스크립트에 이 순서를 담아두면, "새 기능 만들어줘" 한 마디만으로 전체 파이프라인이 실행됩니다.

<hr>

## 워크플로우 1: 기능 개발 파이프라인

가장 일반적인 워크플로우입니다. 새 기능을 처음부터 끝까지 개발하는 전체 과정입니다.

> 💡 **팀 메시지 접두사**(`@cc` 등)는 메시지를 팀(Claude CLI)으로 보내는 표시다. `@cc @서연 ...`처럼 특정 팀원을 지정하거나, 접두사만 쓰면 팀장에게 전달된다. (라우팅 규칙은 팀 CLAUDE.md 참고)

이 워크플로우는 조사와 디자인을 병렬로 진행한 뒤 설계하고 구현한 다음 리뷰와 보고로 이어집니다. 아래는 실제 지시와 전개 예시입니다.
...
프로덕션 장애 상황에서의 긴급 대응 워크플로우입니다.

원인 분석 → 핫픽스 구현 → 긴급 리뷰의 3단계로 빠르게 대응합니다.

> 💡 **핫픽스(Hotfix)란?** 프로덕션(실제 서비스)에서 발생한 장애를 즉시 수정하는 긴급 패치입니다. 일반 기능 개발처럼 조사·설계·병렬 작업을 거칠 여유가 없습니다. 원인 분석 → 수정 → 최소 검증 → 즉시 배포 순서로 속도를 최우선합니다.
...
PR이 올라올 때마다 자동으로 리뷰를 수행하는 워크플로우입니다.

사용자가 PR 리뷰를 지시하면 지훈의 변경 분석과 태양의 코드 리뷰가 병렬로 실행되고, 결과를 모아 GitHub PR 코멘트로 자동으로 답니다.
...
매일 정해진 시간에 각 팀원의 작업 현황을 수집하여 보고하는 워크플로우입니다.

팀장이 다섯 팀원에게 한꺼번에 현황을 묻고, 돌아온 보고를 모아 사용자에게 단일 보고로 전달합니다.

> 💡 **fan-out / fan-in 패턴** fan-out은 하나의 지시를 여러 팀원에게 동시에 보내는 것(1→N), fan-in은 여러 팀원의 응답을 하나로 취합하는 것(N→1)입니다. 팀장이 각 팀원의 보고를 기다렸다가 요약해서 사용자에게 전달하는 일일 보고가 전형적인 fan-out → fan-in 사례입니다.
...
팀 환경 자체를 스크립트로 자동 구성하는 워크플로우입니다.

세션 생성 → 파인 분할 → 레이아웃 → 타이틀 → claude 실행까지 한 스크립트로 팀 화면을 자동 구성합니다. 아래가 그 스크립트입니다.
...
이 스크립트를 실행하면 6개의 파인에 각각 Claude Code가 실행된 팀 환경이 자동으로 구성됩니다.

> 💡 **SESSION="team1"인 이유** TMUX 세션 이름을 `team1`으로 고정하면, `tmux send-keys -t team1:0.4`처럼 모든 스크립트에서 일관된 주소로 팀원에게 메시지를 보낼 수 있습니다. 이름이 다르면 자동화 스크립트 전체를 수정해야 합니다. 실제 운영 세션명은 `team1`입니다.
...
자주 사용하는 워크플로우를 스크립트로 저장해두면 재사용이 편리합니다.
...
> **핵심 정리**: 자동화 워크플로우는 사용자의 단일 지시를 다단계 팀 작업으로 자동 전개한다. 기능 개발, 버그 수정, 코드 리뷰, 일일 보고 등 되풀이되는 일을 워크플로우로 굳혀 두면 팀을 훨씬 수월하게 굴릴 수 있다.
