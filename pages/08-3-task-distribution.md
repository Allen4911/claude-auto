...
6명의 에이전트로 구성된 팀에서 업무를 효율적으로 분담하려면 **역할의 경계**가 명확해야 합니다. 역할이 모호하면 일이 겹치거나, 아무도 안 맡은 일이 생깁니다.
...
![팀장 중심으로 5인 팀원이 담당 업무와 함께 둘러싼 6인 팀 역할 분담도](../assets/08-3-task-distribution-role-map.png)

## 역할 간 협업 흐름 요약

6명이 하나의 작업을 처리하는 기본 흐름은 아래와 같습니다.
...
> 🔀 **네이티브 vs 팀**
>
> 병렬 작업 규모가 작을 때는 Claude Code의 네이티브 병렬 실행(Agent 도구로 서브에이전트를 동시 실행)이 오버헤드 없이 효율적입니다. 그러나 에이전트 수가 늘어나고 각 역할의 전문성이 중요해지는 규모에서는 TMUX 팀처럼 역할이 고정된 분리 인스턴스가 더 안정적입니다. "지훈·수아 병렬"처럼 두세 에이전트 동시 실행은 네이티브로, 6인 팀 전체의 지속 작업은 팀 구성으로 접근하는 것이 실용적입니다. 병렬 실행 원리는 [3-3. 작업 분담과 병렬 실행](03-3-subagent-parallel.md)에서 다룹니다.
...
팀장은 **오케스트레이터**입니다. 직접 구현하지 않고, 작업의 흐름을 관리합니다.
...
각 역할은 작업을 독립적으로 수행하지만, 결과물은 다음 사람에게 넘어갑니다. 이 인수인계가 매끄러우려면 **산출물 경로와 형식**을 미리 정해 둬야 합니다.
...
팀장이 작업을 배분할 때 참조하는 기준표입니다.

![작업 키워드가 담당 팀원으로 연결되고 복합 작업은 팀장이 분배하는 키워드 라우팅 다이어그램](../assets/08-3-task-distribution-keyword-routing.png)
...
실제 작업은 단일 역할로 끝나지 않는 경우가 많습니다. 복합 작업을 분배하는 실전 예시입니다.
...
위 표처럼 서로 독립적인 조사 단계는 병렬로, 앞 결과가 있어야 하는 설계, 구현, 리뷰는 순서대로 나눕니다.

팀장이 실제로 배분할 때의 지시 예시는 다음과 같습니다.
...
긴급한 경우 PM과 디자이너를 건너뛰고 최소 인원으로 처리합니다.
...
Phase 2에서 민준과 태양을 병렬로 배분하는 이유는 두 작업이 서로 독립적이기 때문입니다. 민준이 새 구조를 설계하는 동시에 태양이 현재 코드의 문제점을 정리하면, 두 결과물을 모아 서연에게 넘겨줄 때 더 완전한 정보가 됩니다.
...
지시에 포함된 단어로 어느 팀원에게 배분할지 빠르게 판단합니다.
...
"간단한 헬스체크 API를 만들어줘"라는 지시가 들어왔을 때 팀장이 배분하는 전체 과정입니다.
...
팀원이 작업 완료 후 팀장(Pane 0)에게 보내는 보고는 일정한 형식을 따릅니다. 형식이 일치하면 팀장이 보고를 파싱하거나 요약할 때 오해가 줄어듭니다.
...
실전에서 팀원 간 역할 충돌이 발생할 수 있습니다. 아래 패턴과 대응책을 참고합니다.
