...
이 책에서 구축하는 시스템의 전체 아키텍처를 미리 살펴보자. 각 구성 요소가 어떻게 연결되는지 큰 그림을 먼저 파악하면, 이후 장들을 따라가기가 훨씬 수월합니다.

![전체 시스템 아키텍처](../assets/01-3-architecture.png)

전체 시스템은 4개의 계층으로 나뉩니다. 위에서 아래로 내려갈수록 더 기반에 가까운 레이어입니다.
...
사용자가 팀에게 지시를 내리는 인터페이스 계층입니다. 두 가지 경로를 지원합니다.
...
Remote-Control은 Anthropic API 서버를 경유하여 로컬 세션과 통신합니다. 세션은 항상 로컬 머신에서만 실행되며 로컬 파일시스템과 MCP 서버는 외부로 전송되지 않습니다.
...
팀장(Pane 0)이 사용자의 지시를 수신하고, 분석하여 적절한 팀원에게 분배하는 계층입니다.
...
팀장은 **직접 코드를 작성하거나 파일을 수정하지 않습니다**. 지시를 수령하고, 작업을 분해하고, 팀원에게 배분하고, 결과를 종합하여 보고하는 역할만 수행합니다.
...
각 팀원(Pane 1~5)이 할당받은 작업을 실제로 수행하는 계층입니다.
...
모든 팀원은 **동일한 로컬 파일시스템**에 접근하므로, 한 팀원이 작성한 파일을 다른 팀원이 바로 읽을 수 있습니다.
...
팀장(Pane 0)은 허브 역할을 합니다. 모든 지시는 팀장을 통해 팀원에게 전달되고, 모든 결과는 팀장을 통해 사용자에게 보고됩니다. 팀원끼리 직접 통신하지 않는 "스타 토폴로지(star topology)" 구조입니다.

---

**흐름을 직접 따라가 보기**

아직 환경이 없어도, 이 흐름을 텍스트로 시뮬레이션해볼 수 있습니다.
...
실제 환경에서는 `echo` 대신 Claude Code가 이 역할을 수행합니다.
...
각 장은 이 아키텍처를 한 계층씩 쌓아올리는 순서로 구성됩니다. 2장은 인프라 레이어(기반)를, 3장은 실행 레이어(팀원 파인)를, 4~5장은 제어 레이어(Remote-Control·모바일)를 만듭니다. 6장은 도구를 확장하고, 7장은 오케스트레이션 레이어와 실전 운용을, 8장은 실전 사례와 심화 활용을, 9장은 트러블슈팅과 복구를, 10장은 향후 방향과 커뮤니티를 다룹니다.

2장이 주춧돌(인프라)이라면 9장은 옥상이고, 그 사이 장들이 실행 → 제어 → 도구 → 오케스트레이션 계층을 한 층씩 올립니다. 아래층 없이는 위층이 설 수 없으므로, 순서대로 쌓아야 무너지지 않습니다.

---

**로드맵과 현재 위치 확인**

이 절을 읽는 지금, 우리는 **1장(개요)**을 보고 있습니다. 전체 지도를 펼쳐보는 단계입니다. 다음 장인 2장에서 인프라 레이어를 실제로 구축하는 것부터 시작합니다.

각 장을 마칠 때마다 이 로드맵으로 돌아와 "어느 계층을 완성했는가"를 확인하면 진행 상황을 직관적으로 파악할 수 있습니다.
