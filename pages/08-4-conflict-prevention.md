...
정리하면 충돌은 두 에이전트가 같은 파일을 동시에 수정할 때(파일 충돌), 또는 한쪽 변경이 다른 쪽이 의존하는 인터페이스를 깨뜨릴 때(로직 충돌) 발생한다.
...
앞 단계가 끝난 뒤 다음 단계가 시작되므로, 두 에이전트가 같은 파일을 동시에 건드릴 일이 없다.
...
여섯 단계는 충돌 파일 열기 → 마커 확인 → 올바른 버전 선택 → 마커 제거 → 스테이징 → 커밋 순서다.
...
작업 전·후로 확인하면 대부분의 충돌을 미리 걸러낼 수 있다.
...
이렇게 pre-commit 훅이 커밋 직전 ESLint·Prettier를 자동 적용하므로, 에이전트별 스타일 차이가 커밋에 남지 않는다.
...
동결 공지, 리팩토링, 재동기화 순으로 진행하면 대규모 변경 중에도 충돌을 막을 수 있다.
