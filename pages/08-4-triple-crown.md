...
6장에서 gstack, GSD, superpowers를 각각 소개했습니다. 이 세 도구는 독립적으로도 유용하지만, **셋을 하나의 파이프라인으로 연결**할 때 진가를 발휘합니다. 이것이 Triple Crown 전략입니다.
...
세 도구 없이도 Claude Code는 코드를 작성할 수 있습니다. 하지만 "일단 코드부터" 접근은 프로젝트 규모가 커질수록 방향을 잃기 쉽습니다. Triple Crown은 **전략, 계획, 실행, 검증**을 차례로 밟게 해 이 문제를 막습니다.
...
Triple Crown의 전체 사이클은 5단계로 구성됩니다.
...
코드를 한 줄도 작성하기 전에, **무엇을 왜 만드는지**부터 정합니다. 범위, 우선순위, 기술 방향을 먼저 결정해야 나중에 재작업이 없습니다.

### /autoplan — 개발 계획 자동 생성

요구사항을 설명하면 /autoplan이 CEO 리뷰, 설계 리뷰, 엔지니어링 리뷰를 자동으로 수행하여 실행 계획을 수립합니다.
...
/autoplan은 단순히 태스크를 나열하는 것이 아닙니다. CEO 리뷰, 설계 리뷰, 엔지니어링 리뷰를 자동으로 수행하여 계획의 완성도를 높입니다.

<hr>

## Phase 2: 프로젝트 구조화 (GSD)

/autoplan의 결과를 GSD의 구조화된 프로젝트 관리 체계로 가져옵니다.
...
/autoplan에서 도출된 Phase를 GSD의 페이즈로 변환합니다.
...
GSD의 가장 큰 가치는 이 계획이 `.planning/` 디렉토리에 **파일로 저장**된다는 점입니다. 대화가 끊기거나 세션이 바뀌어도 계획은 유지됩니다.
...
여기서 Triple Crown의 핵심이 드러납니다. GSD가 **무엇을** 할지 관리하고, Superpowers가 **어떻게** 할지를 강제합니다.
...
GSD는 PLAN.md의 태스크를 순서대로 실행하며, 각 태스크의 진행 상태를 추적합니다.

### Superpowers의 품질 보장

GSD 실행 중 각 태스크에서 superpowers 스킬이 자동으로 개입합니다.
...
이 과정은 Red(실패 테스트) → Green(통과 최소 구현) → Refactor(정리) 세 단계가 빙글빙글 돌며 반복됩니다.
...
TDD 스킬이 없으면 Claude는 서버 코드를 먼저 작성하고 "테스트도 추가할까요?"라고 묻습니다. TDD 스킬이 있으면 **반드시 테스트를 먼저 쓴다**.
...
체계적 디버깅은 지켜보고, 가설을 세우고, 확인하고, 고치는 네 단계를 밟습니다.

디버깅 스킬이 없으면 Claude는 코드를 보고 "이 부분이 문제인 것 같다"며 바로 수정을 시도합니다. 디버깅 스킬이 있으면 **가설을 세우고 검증하는 과정**을 거칩니다. 감으로 코드를 고치다 다른 데가 깨지는 악순환을 막습니다.

### 시나리오 3: 병렬 작업 시 — 에이전트 분배 스킬

팀 환경에서 여러 태스크를 동시에 진행할 때 사용합니다.
...
GSD 검증은 "계획한 것을 다 했는가"를 확인하고, gstack 검증은 "만든 것이 제대로 동작하는가"를 확인합니다. 관점이 다르기 때문에 둘 다 필요합니다.
...
같은 기능을 Triple Crown 없이 만드는 경우와 비교합니다.

### 단순 개발
소규모 작업(파일 하나 수정, 간단한 스크립트)에는 Triple Crown이 과합니다. 하지만 **여러 파일에 걸친 기능 개발, 팀 단위 프로젝트, 프로덕션 배포가 필요한 작업**에서는 처음 들인 시간을 나중에 손볼 일이 줄어드는 것으로 메웁니다.

<hr>

## 적용 기준

모든 작업에 Triple Crown을 적용할 필요는 없습니다. 상황에 따라 단계를 선택합니다.
...
Triple Crown 전략은 gstack(전략·검증), GSD(구조·실행), Superpowers(품질·방법론)를 하나의 파이프라인으로 연결합니다. gstack으로 무엇을 왜 만드는지 정하고, GSD로 어떤 순서로 진행할지 관리하고, Superpowers로 각 단계를 제대로 수행합니다. 팀 환경에서는 각 Phase를 역할에 맞는 팀원에게 배분하여 병렬로 실행할 수 있습니다. 모든 작업에 적용할 필요는 없지만, 규모가 큰 프로젝트에서는 처음 구조를 잡느라 쓴 품을 손볼 일이 줄면서 충분히 되찾습니다.
