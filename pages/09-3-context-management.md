...
사용량이 70% 경계선을 넘으면 오래된 내용이 밀려나기 시작하므로, 그 전에 정리해야 합니다.
...
정리하면 context-save로 저장하고 /clear로 비운 뒤 context-restore로 되살리는 흐름입니다.
...
이렇게 Phase가 끝날 때마다 저장·초기화하고 다음 Phase를 시작하면 컨텍스트가 누적되지 않습니다.
...
네 전략을 함께 쓰면 컨텍스트 소비를 크게 늦출 수 있습니다.
...
5분 루프마다 각 pane의 작업·컨텍스트·rate limit·hang 여부를 점검하고, 이상이 있으면 민준에게 보고합니다.
...
이 네 단계만으로도 중단 지점을 빠르게 복원할 수 있습니다.
