...
| 커맨드 | 역할 | 용도 |
|:---|:---|:---|
| `/cso` | Founder / CEO | 제품 비전, 범위, 우선순위 결정 |
...
```
1단계: /cso
   → "사용자 인증 기능을 추가하려 합니다. 비즈니스 관점에서 검토해주세요."
   → Claude가 CEO 역할로 기능의 비즈니스 가치와 우선순위를 분석합니다.

2단계: /autoplan
   → "JWT 기반 사용자 인증 시스템 구현 계획을 작성해주세요."
...
| 버전 | 주요 변경사항 |
|:---|:---|
| v1.1.3.0 | `/checkpoint` → `/context-save` + `/context-restore` 분리 |
| v1.1.1.0 | `/ship` 버전 드리프트 감지 및 수정 |
| v1.1.0.0 | 로컬 HTML 렌더링 및 Retina 스크린샷 지원 |
| v1.0.0.0 | V1 프롬프트 스타일 개선, `/plan-tune` 추가 |

> **현재 최신 버전**: v1.1.3.0 (2026-04-19 기준)
