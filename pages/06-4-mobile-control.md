## 06-4. 모바일에서 지시 전달 및 도구 승인

## 이 장에서 배울 내용

세션에 연결한 뒤 모바일에서 할 수 있는 핵심 작업은 두 가지입니다. 텍스트나 음성으로 Claude에게 지시를 보내는 것, 그리고 Claude가 파일 수정이나 명령어 실행을 요청할 때 스마트폰에서 바로 승인하거나 거부하는 것입니다.

> 💡 **스마트폰이 Claude의 리모컨이 됩니다.** 지금까지 Claude에게 지시하려면 PC 앞에 앉아야 했습니다. 이 장을 익히면 스마트폰 한 대로 어디서든 지시를 전달하고, Claude가 파일을 수정하거나 명령어를 실행하려 할 때 승인 버튼 하나로 허가합니다.
...
> 💡 무선 인터폰을 떠올리면 쉽습니다. 모바일 앱은 집 밖에서 누르는 인터폰 버튼, PC의 Claude는 현관 안에서 소리를 듣는 사람입니다. 버튼을 누르는 즉시 상대방에게 전달되고, 응답도 바로 돌아옵니다.
...
> 💡 Claude가 파일을 바꾸거나 명령을 실행하면 실제로 컴퓨터에 영향을 줍니다. 그래서 위험할 수 있는 작업은 실행 전에 사용자에게 먼저 허락을 구합니다. 이 승인 단계 덕분에 외출 중에도 안전하게 통제할 수 있습니다.

인터넷 뱅킹에서 큰 금액을 이체할 때 "정말 보내시겠습니까?" 확인창이 뜨듯, Claude도 위험한 작업 전에 그 확인창을 여러분의 스마트폰으로 보냅니다.
...
카드 한 장에 무슨 도구로 무엇을 왜 하려는지가 모두 담기고, 그 아래 거부·승인 버튼이 놓입니다. PC 화면을 보지 않고도 이 카드만으로 Claude가 지금 무슨 일을 하려는지 파악하고 곧바로 결정할 수 있습니다.
...
두 버튼은 결과가 정반대로 갈립니다. **승인**을 누르면 그 명령이 PC에서 곧장 실행되고, **거부**를 누르면 실행 대신 "허용하지 않음"이 Claude에게 전달되어 Claude가 다른 방법을 제안하거나 방향을 틉니다. 거부는 작업을 그냥 막는 게 아니라, 다른 길로 가라고 일러 주는 신호입니다.

> 💡 **거부 후 대화를 이어가세요.** 거부 직후 입력창에 "왜 그 명령어를 쓰려 했어?" 또는 "더 안전한 방법은 없어?"라고 물으면 Claude가 맥락을 유지한 채 대안을 제시합니다. 거부해도 대화는 끊기지 않고 그대로 이어집니다.
