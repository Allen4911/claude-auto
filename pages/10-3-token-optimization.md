...
6장에서 RTK로 토큰을 기본적으로 아끼는 법을 다뤘습니다. 이 절에서는 RTK를 넘어서 Claude Code 자체의 설정과 사용 습관까지 손봐 토큰 소비를 최소화하는 심화 전략을 다룹니다. 전략을 조합하면 RTK 단독보다 훨씬 높은 절약률을 달성할 수 있습니다.
...
Claude Code에서 토큰이 소비되는 주요 경로를 이해해야 최적화 포인트를 찾을 수 있습니다.
...
여기서 제일 무거운 건 **도구 결과**와 **대화 히스토리**입니다. 이 둘만 줄여도 전체 토큰 소비의 60~70%를 절감할 수 있습니다.

결국 시스템 프롬프트, 대화 히스토리, 도구 결과, 컨텍스트 파일 이 네 갈래가 최적화의 급소입니다.
...
이 수치를 기준으로 어떤 전략이 가장 효과적인지 판단합니다.

<hr>

## 전략 1: CLAUDE.md 최적화

CLAUDE.md는 매 요청마다 시스템 프롬프트에 포함됩니다. 내용이 길수록 매번 더 많은 토큰이 소비됩니다.
...
프레임워크 이름이나 라이브러리 목록은 `package.json`을 읽으면 알 수 있습니다. CLAUDE.md에는 **코드에서 알 수 없는 규칙과 관례**만 기록합니다.
...
대화가 길어질수록 히스토리가 누적되어 입력 토큰이 증가합니다.
...
대화 중간에 컨텍스트를 압축하여 토큰 사용량을 줄일 수 있습니다.
...
긴 작업 중간에 주기적으로 `/compact`를 실행하면 누적 토큰을 억제할 수 있습니다.
...
위 예처럼 하나의 긴 세션에 모든 작업을 몰면 히스토리가 누적되지만, 작업별로 세션을 분리하면 토큰을 크게 아낄 수 있습니다.

<hr>

## 전략 3: 도구 출력 최적화

도구 실행 결과는 입력 토큰의 가장 큰 부분을 차지합니다.
...
2000줄짜리 파일 전체를 읽으면 약 50,000 토큰이 소비될 수 있습니다. 같은 파일의 특정 구간 20줄만 읽으면 약 500 토큰입니다. 지시만 구체적으로 해도 100배 차이가 납니다.
...
전체 프로젝트 `grep`은 `node_modules` 안의 수천 개 파일까지 스캔해 결과가 수백 줄이 될 수 있습니다. 디렉터리와 확장자를 제한하면 결과가 핵심만 남습니다.

### .claudeignore 활용

불필요한 파일을 Claude의 탐색 범위에서 제외합니다.
...
특히 `node_modules/`, `dist/` 같은 대용량 디렉토리를 제외하면 `ls`나 `find` 명령어의 출력이 크게 줄어듭니다.
...
전후 숫자 차이가 Claude에게 전달되는 출력 크기 차입니다.

<hr>

## 전략 4: 프롬프트 캐싱 활용

Anthropic API는 프롬프트 캐싱을 지원합니다. Claude Code에서는 자동으로 적용되지만, 원리를 알아 두면 더 잘 써먹을 수 있습니다.
...
CLAUDE.md를 자주 수정하면 캐시가 무효화되어 비용이 증가합니다. 안정적인 내용은 CLAUDE.md에, 자주 변하는 지시는 대화에서 직접 전달하는 것이 유리합니다.
...
6개의 Claude Code 인스턴스를 동시 실행하면 토큰 소비가 6배로 늘어납니다. 팀 환경에서의 토큰 관리 전략입니다.

### 필요한 파인만 활성화

모든 팀원이 항상 필요한 것은 아닙니다.
...
예를 들어 백엔드 API만 작성하는 스프린트라면, 디자이너(수아, Pane 3)는 활성화할 필요가 없습니다. 해당 파인의 Claude를 종료해 두면 그 파인의 토큰 소비가 0이 됩니다.

### 역할별 CLAUDE.md 최적화

각 팀원의 CLAUDE.md를 역할에 맞게 최소화합니다.
...
PM에게는 아키텍처 문서 경로만, 개발자에게는 빌드 명령어만 포함하는 식으로 각 역할에 필요한 최소한의 정보만 담습니다.
...
RTK 훅은 각 파인에서 독립적으로 동작합니다. 모든 파인에 RTK가 적용되어 있는지 확인합니다.
...
대화 중 `/cost` 명령어로 현재 세션의 토큰 사용량을 확인할 수 있습니다.
...
RTK 히스토리를 파일로 저장해 두면 주간 절감 추이를 확인할 수 있습니다.
...
이 전략들은 겹쳐 쓸 수 있습니다. RTK + .claudeignore + CLAUDE.md 간소화만 적용해도 전체 토큰을 절반 밑으로 떨어뜨릴 수 있습니다.

<hr>

**전략 적용 우선순위**

한꺼번에 모든 전략을 적용하려 하면 부담이 큽니다. 아래 순서로 하나씩 적용하면서 효과를 확인하는 것을 권장합니다.
...
위 표처럼 전략마다 절약률이 다르며, RTK·.claudeignore·CLAUDE.md 간소화만 겹쳐 써도 전체 소비를 절반 밑으로 떨어뜨릴 수 있습니다.

<hr>

## 요약

토큰 최적화는 RTK 같은 도구 적용만으로 끝나지 않습니다. CLAUDE.md를 간결하게 유지하고, 대화 세션을 작업 단위로 분리하고, 도구 출력 범위를 제한하고, .claudeignore로 불필요한 파일을 제외하는 종합적인 접근이 필요합니다. 팀 환경에서는 필요한 파인만 활성화하고 역할별로 최적화된 설정을 적용하면 6배의 토큰 소비를 효율적으로 관리할 수 있습니다. `/cost`와 `rtk gain`으로 주기적으로 소비량을 확인하면서 어느 전략이 가장 효과적인지 데이터로 판단하는 것이 핵심입니다.
