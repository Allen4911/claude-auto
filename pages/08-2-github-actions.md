...
> 💡 **CI/CD란?** CI(지속적 통합)는 코드가 합쳐질 때마다 자동으로 빌드·테스트해 문제를 즉시 잡는 것, CD(지속적 배포)는 통과한 코드를 자동으로 배포하는 것입니다. 사람이 매번 손으로 테스트·배포하던 일을 GitHub가 대신 해 줍니다.

멀티에이전트 팀과 GitHub Actions를 결합하면 강력한 자동화가 완성됩니다. 서연(개발자)이 코드를 푸시하는 순간 테스트가 돌고, 통과하면 곧바로 배포됩니다. 태양(리뷰어)은 PR에 자동으로 코드 품질 보고서가 달린 상태로 리뷰를 시작합니다.
...
알림을 위해 GitHub Secrets에 webhook URL을 등록합니다.
...
> 💡 **GitHub Secrets란?** 토큰·비밀번호 같은 민감 정보를 코드에 직접 쓰지 않고 GitHub에 안전하게 보관하는 금고입니다. 워크플로우에서는 `${{ secrets.이름 }}`으로 꺼내 쓰며, 로그에는 자동으로 가려져(마스킹) 노출되지 않습니다.
...
멀티에이전트 팀이 병렬로 일하듯, GitHub Actions도 job을 동시에 돌려 파이프라인 시간을 줄입니다.
...
개발 중에 빠르게 여러 번 push할 때 이전 실행이 낭비되지 않도록 합니다. 이 설정 하나로 Actions 사용량을 40% 이상 줄일 수 있습니다.

이처럼 ①불필요한 실행 줄이기 ②조기 종료 설정 ③실행 취소 설정 세 가지를 조합하면 Actions 사용량을 효과적으로 절감할 수 있습니다.

> 💡 GitHub Actions는 실행 시간만큼 무료 한도를 소모합니다. 위 세 가지는 "안 돌려도 되는 실행을 안 돌리는" 방법으로, 한도를 아끼는 손쉬운 방법입니다.
