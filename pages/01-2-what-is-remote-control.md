...
대부분의 개발자가 AI 코딩 어시스턴트를 사용하는 방식은 이렇습니다.

```
[개발자] → [AI 챗봇] → [응답]
```

하나의 터미널(또는 웹 인터페이스)에서 하나의 AI와 대화합니다. 질문을 던지고, 답을 받고, 다시 질문합니다. 이 방식에는 몇 가지 근본적인 한계가 있습니다.

1. **단일 컨텍스트** — 하나의 대화창에서 설계, 구현, 리뷰를 모두 처리하면 컨텍스트가 뒤섞입니다
2. **순차 처리** — 한 번에 하나의 작업만 진행할 수 있습니다. 리서치가 끝나야 설계를, 설계가 끝나야 구현을 시작합니다
3. **디바이스 종속** — 터미널을 닫으면 세션이 끊깁니다. 자리를 비우면 작업도 멈춥니다

> **컨텍스트(context)란?** AI가 현재 대화에서 기억하고 있는 내용 전체를 말한다. 한 창에서 여러 주제를 섞으면 이 기억이 뒤엉켜, AI가 엉뚱한 답을 내놓기 쉽다.

세 한계는 사실 하나의 전제에서 나옵니다. **"AI를 한 명만, 한 창에서, 내 앞에서만 쓴다"**는 전제입니다. 컨텍스트가 섞이는 것도, 작업이 줄서서 기다리는 것도, 자리를 뜨면 멈추는 것도 모두 이 전제의 결과입니다. 이 전제를 깨면 세 한계가 한꺼번에 풀립니다. 그것이 다음 절에서 다룰 팀 에이전트 방식입니다.

---

**일대일 방식의 병목을 눈으로 확인하기**

다음 시나리오를 상상해보자. "결제 기능을 구현해줘"라고 하나의 AI에게 맡기면, AI는 다음 작업을 **줄을 세워서** 처리합니다.
...
팀 에이전트는 같은 일을 절반 시간에 끝냅니다.

<hr>

## Remote-Control 팀 에이전트의 개념

이 책에서 제안하는 방식은 완전히 다릅니다.

![팀 에이전트 개념도](../assets/01-2-team-concept.png)

세 가지 기술의 결합이 핵심입니다. 아래 세 기술이 각각 "공간 · 역할 · 원격 지휘"를 담당해 하나의 팀 시스템을 이룹니다.

![TMUX + CLAUDE.md + Remote-Control 세 기술이 결합해 AI 팀 에이전트를 이루는 구조](../assets/01-2-what-is-remote-control-three-pillars.png)

### 1. TMUX

TMUX는 하나의 터미널 안에 여러 개의 독립된 창(파인)을 만들 수 있는 터미널 멀티플렉서입니다. 각 파인에서 별도의 Claude Code 인스턴스를 실행하면, **물리적으로 분리된 여러 AI 에이전트**가 탄생합니다.
...
각 에이전트는 **독립된 컨텍스트**를 가집니다. PM의 대화가 개발자의 코드 작성을 방해하지 않습니다.
...
CLAUDE.md 파일은 Claude Code의 행동 규칙을 정의하는 설정 파일입니다. 각 에이전트에게 서로 다른 역할을 부여할 수 있습니다.
...
각 파인마다 다른 내용의 CLAUDE.md를 두면, 같은 Claude Code 엔진이라도 완전히 다른 역할의 팀원으로 동작합니다.

---

### 3. Remote-Control

Claude Code의 Remote-Control 기능은 로컬에서 실행 중인 세션을 **다른 기기에서 제어**할 수 있게 합니다.
...
이후 **claude.ai/code** 웹사이트나 **Claude 모바일 앱**에서 해당 세션에 접속해 지시를 내립니다. 세션은 로컬 머신에서 계속 실행되므로, 모바일에서 접속해도 로컬 파일시스템과 모든 도구에 접근 가능합니다.
...
이 모든 과정이 **하나의 TMUX 세션** 안에서 돌아가고, **Remote-Control**로 어디서든 개입할 수 있습니다.

여기서 눈여겨볼 것은 **시간의 흐름**입니다. 1번(출근길)과 6번(사무실 도착)은 사람이 직접 움직인 시간이지만, 2~5번은 그 사이에 팀이 알아서 굴러간 시간입니다. 지시 한 번을 던져두면 이동하는 동안 분석 → 설계 → 구현 → 리뷰가 차례로 진행되고, 도착했을 땐 결과만 확인하면 됩니다. 사람이 자리를 비운 시간이 곧 팀의 작업 시간이 됩니다. 일대일 방식과 가장 다른 점입니다.

---

**시나리오를 따라 직접 체험해보기**

아직 환경이 구축되지 않았더라도, 아래 순서를 머릿속으로 그려보자. 2장부터 이 흐름을 직접 구현하게 됩니다.
...
6단계 흐름이 자연스럽게 그려진다면, 이 책을 읽을 준비가 된 것입니다.
...
- **로컬 실행**: 세션은 항상 로컬 머신에서 실행됩니다. 클라우드로 이전되지 않습니다
- **인바운드 포트 없음**: 로컬 머신에 포트를 열 필요가 없습니다. 보안에 유리합니다
- **자동 재연결**: 네트워크가 일시적으로 끊겨도 복구되면 자동으로 재연결됩니다
- **실시간 동기화**: 모바일에서 보내는 메시지가 즉시 로컬 세션에 반영됩니다
...
이 메시지가 표시되면 모바일 앱이나 웹에서 해당 세션에 접속할 수 있습니다. 4장에서 이 과정을 직접 따라합니다.
