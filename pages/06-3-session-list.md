## 06-3. 세션 목록에서 팀 에이전트 선택

## 이 장에서 배울 내용

QR 코드 없이도 세션 목록 화면에서 원하는 세션을 직접 찾아 연결할 수 있습니다. 특히 여러 팀 에이전트를 동시에 운영할 때, 세션 목록을 통해 각 에이전트의 상태를 한눈에 확인하고 빠르게 전환하는 방법을 배웁니다.

세션 목록은 지금 몇 명의 에이전트가 일하고 있는지, 누가 입력을 기다리는지, 누가 멈췄는지를 한 화면에 보여줍니다. QR 코드가 첫 연결이라면, 세션 목록은 팀 전체를 내려다보는 현황판인 셈입니다.
...
> 💡 **Code 탭이 안 보인다면?** 구독 등급(Pro 이상)이나 앱 버전을 확인하세요(6-1장 준비 사항 참고). 탭이 있지만 눌러도 반응이 없다면 앱을 완전히 종료 후 재시작합니다.
...
> 💡 여기서 **세션**은 PC에서 돌아가는 Claude Code 하나, 곧 팀원 한 명입니다. 점의 색만 봐도 그 세션이 지금 일하는 중인지, 내 입력을 기다리는지 바로 알 수 있습니다.
...
외출 중에 팀을 점검할 때는 이렇게 합니다. Code 탭을 열어 세션 목록을 쭉 훑은 다음, 초록 점이 뜬 세션은 작업 중이니 부제목으로 진행 상황만 확인하고 넘어갑니다. 빈 점 세션은 입력 대기 상태이니 탭해서 다음 지시를 보내고, 노란 점 세션은 도구 승인을 기다리고 있으니 탭해서 실행을 승인합니다(6-4장). 회색 세션은 오프라인이라 PC 상태를 따로 살펴야 합니다. 다 처리했으면 목록으로 돌아가 다음 세션으로 넘어갑니다.
...
| 노란 점 + "도구 승인 필요" | 실행 권한 승인 대기 | 6-4장 참고 |
| 회색 + "오프라인" | PC 세션이 종료되었거나 네트워크 불량 | PC 상태 확인 |

점 색깔은 신호등처럼 읽으면 됩니다. 초록은 정상 동작, 노랑은 도구 승인처럼 내 확인이 필요한 상태(6-4장에서 다룹니다), 회색은 세션이 꺼졌거나 네트워크가 끊긴 오프라인 상태입니다. 노랑이 뜨면 한 번 손봐 줘야 한다는 신호로 받아들이면 됩니다.
