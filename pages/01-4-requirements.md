...
이 챕터에서 구축하는 멀티에이전트 환경을 원활하게 실행하려면 일정 수준의 하드웨어와 소프트웨어가 필요합니다. 시작 전에 본인의 환경이 요건을 충족하는지 확인하자. 체크리스트를 따라 실행하면 5분이면 준비 여부를 알 수 있습니다.
...
숫자로 따져보면 팀 자체가 쓰는 2.4GB는 16GB의 일부일 뿐입니다. 남는 메모리에서 Node.js 빌드·테스트·브라우저가 함께 돕니다. 그래서 8GB로도 팀은 뜨지만, 무거운 작업을 몇 개 겹치는 순간 금세 빠듯해집니다. 16GB를 권장하는 이유가 바로 이 여유 공간에 있습니다.
...
`total`이 8GB(표시: `7.7Gi` ~ `8Gi`) 이상이면 최소 사양을 충족합니다.
...
본인의 운영체제에 해당하는 항목만 따라 실행하면 됩니다. 출력된 버전·용량이 앞의 요건을 충족하는지 비교하자.
...
모든 항목이 최소 버전 이상이면 다음 챕터로 진행합니다. 미설치 항목이 있다면 2장에서 설치 방법을 안내합니다.
...
표를 길이로 바꿔 보면 전체 시간의 절반 안팎이 **WSL2 + Ubuntu 설치**(Windows 사용자 한정)에 쏠려 있습니다. 나머지 단계는 대부분 명령어 한두 줄이라 금방 끝납니다. 대기 시간은 초반 OS 준비에 몰려 있고, 그 고비만 넘기면 이후는 빠르게 흘러갑니다. macOS·네이티브 리눅스 사용자라면 이 단계가 통째로 빠져 20~30분이면 충분합니다.
