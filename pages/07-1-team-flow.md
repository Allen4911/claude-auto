...
지시 하나는 아래 5개 Phase를 순서대로 거친다. 수신부터 보고까지 한 줄기로 이어지는 파이프라인이다.
...
> 💡 **분해 기준 두 가지** — 팀장은 ①**담당 역할**: 어떤 종류의 작업인가(설계·조사·구현·리뷰), ②**의존성**: 이 작업이 시작하려면 먼저 끝나야 할 다른 작업이 있는가를 반드시 확인한다. 이 두 기준으로 분해하면 병렬/순차 배분이 자연스럽게 정해진다.
...
> 💡 **지시문 작성 요령** — `tmux send-keys`로 전달하는 지시는 한 줄로 압축하거나 `\`로 이어 쓴다. 줄바꿈이 섞이면 중간에 잘려서 팀원이 불완전한 지시를 받는다. **입력(어떤 파일/데이터)**, **처리(무엇을 해라)**, **출력(결과를 어디에 저장)** 세 요소를 포함하면 팀원이 혼동 없이 작업할 수 있다.
...
5분 이상 응답이 없거나 화면이 바뀌지 않으면 파인 재기동을 검토한다. 팀장은 진행 관리 도중 사용자에게 **중간 보고**를 보내 대기 상황을 알려 주는 것이 좋다.
...
보고는 항상 **무엇이 완료됐는지(결과)**와 **다음 단계가 있다면 무엇인지(후속 조치)**를 함께 담는 것이 좋다. 사용자가 후속 지시를 내릴 때 맥락이 이어지기 때문이다.
...
> 💡 이 CLAUDE.md가 팀장 파인에 로드되면, 팀장 Claude는 사용자 지시를 받을 때마다 이 규칙에 따라 자동으로 분석·배분·보고 흐름을 따른다. "직접 하지 말고 팀원에게 위임하라"는 행동 원칙이 자동화되는 것이다.
...
> 💡 **설계 팁**: 팀을 처음 구성할 때 CLAUDE.md를 한 번 제대로 작성해 두면, 이후 지시만 바꿔도 팀이 자동으로 올바른 흐름으로 움직인다. CLAUDE.md는 팀의 행동 설명서다.
