## 10-2. TMUX 세션 복구 및 재연결

## 이 절에서 배우는 것

TMUX 기반 팀 환경을 운용하다 보면 세션이 끊기거나, 파인이 사라지거나, 레이아웃이 깨지는 상황이 발생합니다. 이 절에서는 상황별 원인을 파악하고 팀 세션을 빠르게 복구하는 단계별 방법을 다룹니다.
...
위험도별로 복구 난이도가 다르입니다. **낮음**은 재연결만 하면 되고, **중간**은 해당 파인만 재시작하면 되며, **높음**은 셋업 스크립트로 전체를 처음부터 다시 구성해야 합니다.

![TMUX 복구 상황 분류 트리 — 문제 발생에서 SSH 연결 끊김(낮은 위험)/Claude만 종료(중간 위험)/TMUX 서버 크래시(높은 위험)/시스템 재부팅(높은 위험) 4가지로 분기, 각 분기에 복구 명령(tmux attach·claude 재실행·setup-team.sh)](https://raw.githubusercontent.com/Allen4911/claude-auto/main/assets/10-2-tmux-recovery-situation-tree.png)

<hr>

## 상황 1: SSH 연결이 끊어졌을 때

가장 흔하고 가장 안전한 상황입니다. TMUX 세션은 SSH 연결과 독립적으로 유지됩니다.
...
세션 이름(`team`)과 생성 시각이 표시되면 세션이 살아있는 것입니다. 아무것도 출력되지 않으면 세션이 없는 것이므로 상황 4(전체 재구성)로 넘어갑니다.
...
연결만 끊어진 것이므로 모든 파인의 Claude Code는 그대로 실행 중입니다. 아무런 데이터 손실 없이 그대로 이어서 작업할 수 있습니다.

위 두 단계처럼 SSH는 끊겨도 서버의 TMUX 세션은 살아있으므로, `tmux attach`로 재연결하면 데이터 손실 없이 그대로 이어집니다.

<hr>

**자동 재연결 alias 설정 (선택 사항)**

SSH 재접속 후 바로 세션에 붙는 alias를 `~/.bashrc`에 추가하면 편리합니다.
...
이후 SSH 재접속 시 `team` 한 단어로 바로 세션에 복귀할 수 있습니다.

<hr>

## 상황 2: 특정 파인의 Claude가 종료되었을 때

한 팀원의 Claude Code가 종료되었지만 파인 자체는 살아있는 경우입니다.

### 진단

파인에서 Claude Code가 종료되면 일반 쉘 프롬프트(`$`)가 표시됩니다.
...
출력에 `$` 프롬프트가 보이면 Claude Code가 종료된 상태, `❯` 또는 `claude>` 프롬프트가 보이면 정상 실행 중입니다.
...
Claude Code는 이전 대화 히스토리를 로컬에 저장하므로, 재실행 후 `/resume` 명령으로 이전 세션을 이어갈 수 있습니다.
...
즉 진단 반복문으로 비활성 파인을 찾은 뒤 해당 파인에서 claude를 재실행(필요 시 `/resume`)하면 복구됩니다.

<hr>

## 상황 3: 파인 자체가 사라졌을 때

TMUX 파인이 닫히면 복구할 수 없습니다. 새 파인을 생성하고 재설정해야 합니다.
...
인덱스 번호가 0, 1, 2, 4, 5로 건너뛰면 3번 파인이 사라진 것입니다.
...
파인을 새로 나누고 타이틀을 붙인 뒤 claude를 띄우고 레이아웃을 정리하면 됩니다.

<hr>

**파인 2개 이상 사라진 경우**

파인이 2개 이상 사라지면 레이아웃이 복잡하게 엉킬 수 있습니다. 이 경우는 아래 상황 4(전체 재구성)의 셋업 스크립트를 사용하는 것이 더 빠르입니다.
...
시스템 재부팅이나 TMUX 서버 크래시로 세션이 완전히 사라진 경우입니다. 셋업 스크립트로 전체를 재구성합니다.
...
출력이 "세션 없음"이거나 빈 줄이면 전체 재구성이 필요합니다.
...
셋업 스크립트는 TMUX 세션 생성, 파인 분할, 타이틀 설정, Claude Code 실행을 자동으로 수행합니다.

tmux ls로 소실을 확인하고 setup-team.sh를 돌린 뒤 세션에 접속하면 됩니다. 스크립트가 세션·파인·역할을 한 번에 재구성합니다.
...
수동 복구가 번거롭다면 상태를 점검하고 자동 복구하는 스크립트를 작성할 수 있습니다.
...
위 스크립트는 ①세션 존재 → ②파인 수 → ③각 파인의 Claude 실행 상태를 차례로 점검하고, 빠진 부분만 자동으로 복구합니다.
...
이 스크립트를 cron에 등록하면 주기적으로 팀 상태를 점검할 수 있습니다.
...
파인 수는 맞지만 레이아웃이 깨진 경우입니다. 파인이 겹치거나 크기가 이상해진 경우에 적용합니다.
...
위 명령들로 main-vertical 레이아웃을 다시 적용하고 Pane 0 너비·타이틀 표시를 복원하면 깨진 배치가 정상으로 돌아옵니다.
...
tmux-resurrect 플러그인을 사용하면 세션 상태를 파일로 저장하고 복원할 수 있습니다.
...
Claude Code는 메모리를 꽤 먹습니다. 여섯 개를 한꺼번에 돌리면 부담이 만만치 않습니다.
...
각 Claude 인스턴스가 500MB 안팎을 사용한다면, 6개를 동시 실행하면 약 3GB가 필요합니다. 시스템 총 RAM이 8GB 이하라면 OOM 위험이 있습니다.

메모리가 모자라 Claude가 OOM으로 죽는 걸 막으려면 RAM을 충분히 확보하거나(16GB 이상 권장), 동시 실행 파인 수를 줄이는 것을 고려합니다.
...
정리하면 예방은 두 갈래입니다. ①앞서 설치한 tmux-resurrect 플러그인으로 세션 상태를 저장·복원해 두고(`Ctrl-b`+`Ctrl-s`로 저장, `Ctrl-b`+`Ctrl-r`로 복원), ②위 `ps aux` 명령으로 Claude 프로세스의 메모리 사용량을 주기적으로 점검해 OOM 종료를 미리 막습니다.

<hr>

## 요약

TMUX 세션 복구의 핵심은 **단계별 진단**입니다. SSH가 끊기면 다시 붙고, Claude만 죽으면 재실행하고, 파인이 사라지면 새로 만들고, 세션이 통째 날아가면 셋업 스크립트로 다시 짭니다. `check-team.sh` 같은 자동 점검 스크립트와 tmux-resurrect 플러그인을 활용하면 복구 시간이 확 줄어듭니다. OOM을 방지하려면 `free -h`와 `ps aux`로 메모리 여유량을 주기적으로 확인하는 습관이 중요합니다.
