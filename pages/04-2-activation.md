...
> 💡 **비유:** 설정 파일(방법 1)은 현관 자동잠금 장치처럼 항상 작동합니다. CLI 플래그(방법 2)는 그때그때 열쇠를 꽂는 것이고, 슬래시 명령(방법 3)은 이미 집에 들어온 뒤 잠금을 거는 것입니다.
...
> 💡 세션 이름은 `--remote-control` 옵션 바로 뒤에 공백으로 구분해 입력합니다. 이름을 생략하면 랜덤 이름이 자동 배정됩니다.
