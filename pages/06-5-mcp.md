...
**MCP(Model Context Protocol)**는 Anthropic이 공개한 오픈 표준으로, AI 모델과 외부 도구·서비스를 표준화된 방식으로 연결하는 프로토콜입니다.

> 💡 **USB에 비유하면?** 예전에는 기기마다 전용 케이블이 필요했지만 USB라는 표준이 나오면서 하나의 포트로 모든 걸 꽂게 됐습니다. MCP는 AI용 USB입니다. Claude에 "표준 포트" 하나만 있으면 파일시스템·GitHub·데이터베이스 같은 도구를 규격대로 꽂아 쓸 수 있습니다. 도구가 달라도 연결 방식은 항상 동일합니다.
...
MCP 서버는 크게 두 가지를 Claude에게 제공합니다.
...
> **핵심 요약**: MCP는 Claude Code와 외부 도구를 연결하는 표준 인터페이스입니다. 공식 MCP 서버를 활용하거나, 직접 커스텀 MCP 서버를 만들어 팀 에이전트의 능력을 무한히 확장할 수 있습니다.
