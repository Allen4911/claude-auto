...
**MCP(Model Context Protocol)**는 Anthropic이 공개한 오픈 표준으로, AI 모델과 외부 도구·서비스를 표준화된 방식으로 연결하는 프로토콜입니다. USB가 다양한 장치를 컴퓨터에 연결하듯, MCP는 파일 시스템·GitHub·Slack 등 다양한 외부 도구를 Claude Code에 연결합니다.
...
> **핵심 요약**: MCP는 Claude Code와 외부 도구를 연결하는 표준 인터페이스입니다. Filesystem·GitHub·Slack 등 공식 MCP 서버를 활용하거나, 직접 커스텀 MCP 서버를 만들어 팀 에이전트의 능력을 무한히 확장할 수 있습니다.
