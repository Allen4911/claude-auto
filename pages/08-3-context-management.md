...
사용량이 70% 경계선을 넘으면 오래된 내용이 밀려나기 시작하므로, 그 전에 정리해야 한다.
...
정리하면 ①context-save로 상태 저장 → ②/clear로 초기화 → ③context-restore로 복원의 3단계다.
...
bash 내부전달스크립트.sh 4 "/context-save"
...
이렇게 Phase가 끝날 때마다 저장·초기화하고 다음 Phase를 시작하면 컨텍스트가 누적되지 않는다.
...
## 현재 프로젝트
- 경로: /path/to/your-project
- Phase: 3 (현재 진행 중인 기능)
- 완료된 것: P1(기능 A), P2(기능 B)
...
지금까지의 네 전략(작업 세분화 · 필요한 부분만 읽기 · 출력 제한 · CLAUDE.md 활용)을 함께 쓰면 컨텍스트 소비를 크게 늦출 수 있다.
...
```bash
# 이상 발견 시 민준에게 보고
bash 내부전달스크립트.sh 1 "서연(P4) 컨텍스트 82%. context-save 후 /clear 처리 필요."
```

즉 5분 루프마다 각 pane의 작업·컨텍스트·rate limit·hang 여부를 점검하고, 이상이 있으면 민준에게 보고하는 흐름이다.
...
저장 위치: `~/memory/30_Daily/YYYY-MM-DD.md`
...
위 네 단계(마지막 커밋 확인 → 브랜치 확인 → 미완료 작업 검색 → 테스트 실행)만으로도 중단 지점을 빠르게 복원할 수 있다.
