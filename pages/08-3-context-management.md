...
Claude Code와 장시간 작업하다 보면 "컨텍스트가 가득 찼다"는 신호를 받게 됩니다. 이 절에서는 컨텍스트가 무엇인지, 왜 관리해야 하는지, 그리고 멀티에이전트 팀에서 어떻게 효율적으로 관리하는지를 단계별로 배웁니다.
...
> 💡 **컨텍스트(Context)란?** 사람으로 치면 "단기 기억"입니다. 대화를 나눌수록, 파일을 읽을수록 기억 공간이 채워집니다. 책 한 권 분량의 정보를 한꺼번에 외우려 하면 앞의 내용을 잊듯, AI도 컨텍스트가 가득 차면 초반의 중요한 내용을 "잊어버립니다".
...
![컨텍스트 소비 흐름과 70% 경계선 overflow](../assets/08-3-context-management-consume-flow.png)
...
> 💡 **토큰(Token)이란?** 컨텍스트 용량의 단위입니다. 영어 단어 하나가 약 1토큰, 한국어 한 글자가 약 1~2토큰입니다. `git log` 전체 출력은 수천 토큰을 한 번에 소비할 수 있습니다. Claude Sonnet 4.6의 컨텍스트 한도는 200,000 토큰(약 15만 단어 분량)입니다.
...
> 💡 **RTK가 자동으로 작동하는 원리** RTK는 Claude Code의 "훅(Hook)" 기능을 이용합니다. 여러분이 `git log`를 실행하면, Claude Code가 명령어를 실행하기 전에 RTK에 먼저 전달합니다. RTK가 출력을 압축한 결과만 Claude Code에 보이므로, 여러분이 따로 설정하지 않아도 자동으로 절약됩니다.
...
### context-save → /clear → context-restore 절차

컨텍스트를 안전하게 초기화하고 작업을 이어가는 방법입니다.

**1단계: 현재 작업 상태 저장**
```bash
/context-save
```
현재 진행 중인 작업, 중요한 결정 사항, 남은 작업 목록, git 상태 등이 파일에 저장됩니다.

**2단계: 컨텍스트 초기화**
```bash
/clear
```
대화 기록이 모두 지워지며 컨텍스트가 0%로 리셋됩니다. 저장한 파일은 그대로 남아 있습니다.

**3단계: 새 세션에서 이전 작업 복원**
```bash
/context-restore
```
저장해 둔 파일을 읽어 작업 맥락을 복원합니다. 마치 책갈피를 꽂아 두고 다시 펼치는 것과 같습니다.

![context-save→clear→restore 3단계 흐름도](../assets/08-3-context-management-save-restore.png)
...
![Phase 단위 컨텍스트 초기화 반복 사이클](../assets/08-3-context-management-phase-cycle.png)
...
> 💡 **`sed -n '50,80p'`란?** sed는 텍스트를 다루는 도구입니다. `-n '50,80p'`는 "50번째 줄부터 80번째 줄만 출력하라"는 뜻입니다. 1000줄짜리 파일에서 필요한 30줄만 읽으면 토큰을 97%나 절약할 수 있습니다.
...
> 💡 **CLAUDE.md가 왜 유용할까요?** `/clear`로 컨텍스트를 초기화하면 대화 기록은 모두 사라지지만, CLAUDE.md 파일은 남아 있습니다. Claude Code는 새 세션을 시작할 때 CLAUDE.md를 자동으로 읽으므로, 역할·규칙·프로젝트 정보는 항상 기억하게 됩니다.

![4가지 컨텍스트 절약 전략 카드 인포그래픽](../assets/08-3-context-management-strategies.png)
...
![5분 체크 루프 팀원 컨텍스트 모니터링 흐름](../assets/08-3-context-management-monitor-loop.png)
...
> 💡 **컴팩션(Compaction)이란?** "압축"을 뜻합니다. 대화 내역이 너무 길어지면 Claude Code가 자동으로 초반 내용을 한 줄 요약으로 바꿔 공간을 확보합니다. 책의 긴 챕터를 한 줄로 요약하는 것과 비슷합니다. 요약 과정에서 세부 내용은 사라질 수 있어, **70% 규칙을 지켜 사전에 context-save + /clear를 실행하는 것이 더 안전**합니다.
...
![비상 컨텍스트 복구 4단계 순서도](../assets/08-3-context-management-recovery-steps.png)
