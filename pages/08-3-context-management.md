...
정리하면 context-save로 저장하고 /clear로 비운 뒤 context-restore로 되살리는 흐름이다.
...
네 전략을 함께 쓰면 컨텍스트 소비를 크게 늦출 수 있다.
...
5분 루프마다 각 pane의 작업·컨텍스트·rate limit·hang 여부를 점검하고, 이상이 있으면 민준에게 보고한다.
...
이 네 단계만으로도 중단 지점을 빠르게 복원할 수 있다.
