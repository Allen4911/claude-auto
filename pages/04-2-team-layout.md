...
> 🔀 **네이티브 vs 팀**: 네이티브 서브에이전트는 부모 세션 안에서 순간적으로 소환되는 분신입니다. 반면 이 절에서 설명하는 TMUX 파인은 각각 독립된 CLI 프로세스로, 세션이 살아있는 동안 역할을 지속합니다. "한 세션 안의 동적 위임"과 "복수 세션의 지속 협업"이라는 구조 차이가 두 방식의 근본 갈림길입니다. → [3-1. Subagent와 멀티에이전트](03-1-subagent-basics.md)
...
1. **세션 생성** — 백그라운드(`-d`)로 팀 세션을 만듭니다. 터미널 실제 크기를 먼저 읽고, 실패하면 기본값(317×85)을 씁니다
2. **파인 분할** — Pane 0에서 시작해 오른쪽으로 5번 분할해 6칸을 만듭니다
3. **레이아웃 정렬** — `even-horizontal`로 먼저 균등 배분한 뒤 `main-vertical`로 전환해 왼쪽 팀장 칸을 넓힙니다. 두 단계로 나누는 이유는 `main-vertical` 직접 적용 시 칸 배열이 불규칙해질 수 있기 때문입니다
4. **제목 표시** — 파인 위쪽 테두리에 제목줄을 켭니다
5. **이름 붙이기** — 각 파인에 담당자 이름을 설정합니다
