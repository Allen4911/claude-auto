...
Remote Control을 사용하다 보면 인증 관련 오류를 마주칠 때가 있습니다. 대부분의 문제는 토큰 만료, 계정 불일치, 네트워크 설정에서 발생합니다. 이 절에서는 자주 발생하는 인증 오류와 해결 방법을 증상별로 단계적으로 정리합니다.
...
처음에는 잘 동작하던 Remote Control이 어느 날 갑자기 연결을 거부하기도 합니다. 대부분은 사용자 실수가 아니라 **시간이 지나 저절로 만료**된 것입니다.
...
모바일에서 세션 목록이 비어있거나 연결 시 "Session not found" 오류가 표시됩니다.
...
12891
```
두 개의 숫자가 나온다면, 프로세스가 2개 실행 중임을 의미합니다. 아무것도 출력되지 않으면 Claude Code가 완전히 종료된 상태입니다.
...
```
모바일 앱에서 로그인된 계정과 이메일이 동일한지 확인합니다. 다르면 어느 한쪽에서 로그아웃 후 같은 계정으로 재로그인합니다.

<hr>

### 2. "Authentication failed" — 인증 실패

연결 시도 시 인증 실패 오류가 발생합니다. 가장 흔한 원인은 토큰 만료입니다.
...
```
`Token expires` 날짜가 오늘보다 이전이라면 만료된 것입니다.
...
브라우저가 자동으로 열리며 Anthropic 계정으로 로그인합니다. 로그인 후 터미널에 표시되는 인증 코드를 입력하면 새 토큰이 발급됩니다.

정리하면 계정 확인, 재인증, 브라우저 로그인 순서입니다.
...
모바일에서 세션을 선택했지만 연결이 되지 않고 타임아웃이 발생합니다. 인증 자체보다 **네트워크 경로** 문제인 경우가 많습니다.
...
Address: 104.18.x.x
```
`Address`가 출력되면 DNS는 정상입니다. 아무것도 나오지 않으면 DNS 서버에 문제가 있습니다.
...
WSL2 환경에서는 Windows 방화벽이 아웃바운드 연결을 막기도 합니다. Windows Defender 방화벽 설정에서 WSL2 관련 규칙을 확인합니다.
...
WSL2에서는 Windows 호스트를 경유해 외부와 통신하므로, Windows 방화벽이 추가 차단을 할 수 있습니다.
...
세션에는 연결되지만 도구 실행 시 "Permission denied"가 반복됩니다. 이는 인증 자체가 아니라 **Claude Code의 권한 설정** 문제입니다.
...
"npm run *" 처럼 와일드카드를 과도하게 사용하면 `npm run test` 같은 무해한 명령어도 차단될 수 있습니다.

**3단계: 규칙 수정**

`deny` 배열에서 지나치게 광범위한 패턴을 찾아 구체적으로 수정합니다.
...
Anthropic 인증 토큰은 일정 기간이 지나면 만료됩니다. 장기간 실행 중인 Claude Code 세션에서 갑자기 인증 오류가 발생한다면 토큰 만료를 의심합니다.
...
즉 로그인 시 발급된 토큰이 만료되면 인증 오류가 나고, `auth login`으로 재발급하면 다시 정상화되는 순환 구조입니다.

<hr>

### 여러 계정 사용 시

팀 환경에서 여러 Anthropic 계정을 사용하는 경우 계정 전환에 주의합니다.
...
팀 세션의 모든 파인이 같은 계정을 사용한다면, 토큰 만료 시 전체가 동시에 실패합니다.
...
위 반복문처럼 토큰이 만료되면 P0~P5 파인이 동시에 실패하므로, 모든 파인을 순차로 재시작해 한꺼번에 복구합니다.

**재인증만 먼저 하는 방법 (빠른 복구):**

별도 터미널에서 재인증하면 토큰 파일이 즉시 갱신됩니다. 실행 중인 Claude Code는 새 토큰을 자동으로 읽으므로, 재시작 없이 복구됩니다.
...
하나의 파인만 문제가 있다면 해당 파인의 Claude Code 프로세스를 개별적으로 재시작합니다.
...
특정 파인만 실패하는 경우, 해당 파인에서 임의로 `claude auth logout`이 실행되었거나 프로세스가 비정상 종료된 경우가 많습니다. 우선 해당 파인의 상태를 화면에서 직접 확인합니다.
...
문제 발생 시 아래 5단계 순서로 진단하면 원인을 빠르게 찾을 수 있습니다.
...
프로세스, 인증, 네트워크, 로그, 설정 파일을 차례로 확인하며 원인을 좁혀갑니다.

<hr>

## 빠른 점검 스크립트

매번 5단계를 수동으로 실행하는 대신, 아래 스크립트를 `~/auth-check.sh`로 저장해 두면 한 번에 전체 상태를 확인할 수 있습니다.
...
Remote Control 인증 문제의 90%는 **토큰 만료**, **계정 불일치**, **네트워크 차단** 중 하나입니다. `claude auth status`로 인증을, `pgrep`으로 프로세스를, `curl`로 연결을 확인하면 대부분 잡힙니다. 반복 발생한다면 `auth-check.sh` 스크립트로 한 번에 전체 상태를 점검하는 습관을 들이면 진단 시간이 확 줄어듭니다.
