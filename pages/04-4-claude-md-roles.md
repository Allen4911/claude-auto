...
> 🔀 **네이티브 vs 팀**: 네이티브 서브에이전트의 역할 정의는 정의 파일에 작성해두더라도 호출할 때마다 새 컨텍스트에서 시작하므로 실행 이력이 누적되지 않습니다. 반면 CLAUDE.md 역할 파일은 세션이 종료될 때까지 컨텍스트에 상주하여 지속적인 페르소나를 유지합니다. 팀이 "기억"을 갖게 하려면 TMUX 팀 방식이 필요한 이유입니다. → [3-2. 역할별 Subagent 설계](03-2-subagent-roles.md)
...
> 💡 **Bot Mode란?** 원격 제어 채널에서 메시지가 올 때 에이전트가 자동으로 알아채고 처리하는 모드입니다. `[채널명:12345]` 같은 접두사가 붙은 메시지를 보면, 에이전트는 "아, 이건 사람이 직접 친 게 아니라 원격 채널에서 온 지시구나"라고 인식하고 처리 후 같은 채널로 결과를 돌려보냅니다. 4장에서 자세히 다룹니다.
