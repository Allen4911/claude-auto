## 8-2. TMUX 세션 복구 및 재연결

TMUX 기반 팀 환경을 운용하다 보면 세션이 끊기거나, 파인이 사라지거나, 레이아웃이 깨지는 상황이 발생한다. 이 절에서는 팀 세션을 빠르게 복구하는 방법을 다룬다.
