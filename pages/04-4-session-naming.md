...
이름은 위에서부터 차례로 "있으면 그걸 쓰고, 없으면 다음으로 내려가는" 방식으로 정해집니다. 직접 지어 준 이름(1·2순위)이 있으면 그대로 쓰고, 없으면 대화 내용에서 의미 있는 문구를 골라(3순위) 붙입니다. 그것마저 없으면 마지막으로 `호스트명-형용사-명사` 꼴의 임의 이름(4순위, 예: `mypc-graceful-unicorn`)이 자동으로 매겨집니다. 즉 가장 명확한 이름이 항상 우선이고, 아무것도 정해 주지 않아도 빈 이름이 되는 일은 없습니다.
