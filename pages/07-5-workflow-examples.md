...
지금까지 팀 구성, Remote-Control, Bot Mode, 업무 분담을 각각 다루었다. 이 장에서는 이 모든 걸 **하나의 자동 워크플로우**로 묶는 실전 예시를 다룬다.

자동화 워크플로우의 핵심은 **사용자가 한 마디만 하면 여러 단계 작업이 알아서 펼쳐진다**는 것이다.
...
이 워크플로우는 조사와 디자인을 병렬로 진행한 뒤 설계하고 구현한 다음 리뷰와 보고로 이어진다. 아래는 실제 지시와 전개 예시다.
...
사용자가 PR 리뷰를 지시하면 지훈의 변경 분석과 태양의 코드 리뷰가 병렬로 실행되고, 결과를 모아 GitHub PR 코멘트로 자동으로 단다.
...
팀장이 다섯 팀원에게 한꺼번에 현황을 묻고, 돌아온 보고를 모아 사용자에게 단일 보고로 전달한다.
