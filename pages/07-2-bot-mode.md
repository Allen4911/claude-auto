...
Bot Mode는 아래 3단계로 한 바퀴 돈다. 사용자의 한 번 지시가 팀장과 팀원을 거쳐 다시 사용자에게 답이 돌아온다.
...
팀장은 팀원들의 보고를 그대로 옮기지 않고 **추려서 요약해** 사용자가 한눈에 알아보게 다듬는다. 기술적 세부사항은 줄이고 "무엇이 됐고, 다음엔 뭘 해야 하는가"로 압축하는 것이 좋다.
...
이 규칙이 CLAUDE.md에 있으면 팀장이 어느 채널에서 지시가 왔는지 알아서 같은 채널로 답한다. 채널 규칙이 없으면 응답 방향을 매번 직접 정해야 해 헷갈린다.
