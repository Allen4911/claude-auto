...
외출 중에 팀을 점검할 때는 이렇게 합니다. Code 탭을 열어 세션 목록을 쭉 훑은 다음, 초록 점이 뜬 세션은 작업 중이니 부제목으로 진행 상황만 확인하고 넘어갑니다. 빈 점 세션은 입력 대기 상태이니 탭해서 다음 지시를 보내고, 노란 점 세션은 도구 승인을 기다리고 있으니 탭해서 실행을 승인합니다(5-4장). 회색 세션은 오프라인이라 PC 상태를 따로 살펴야 합니다. 다 처리했으면 목록으로 돌아가 다음 세션으로 넘어갑니다.
...
점 색깔은 신호등처럼 읽으면 됩니다. 초록은 정상 동작, 노랑은 도구 승인처럼 내 확인이 필요한 상태(5-4장에서 다룹니다), 회색은 세션이 꺼졌거나 네트워크가 끊긴 오프라인 상태입니다. 노랑이 뜨면 한 번 손봐 줘야 한다는 신호로 받아들이면 됩니다.
