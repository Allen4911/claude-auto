...
> 💡 **09-4와의 관계** 09-4에서 소개한 `lock:파일경로` 패턴과 `team:events` pub/sub가 이 절에서 정규화·확장됩니다. 09-4 충돌 체크리스트의 "Redis 확인" 항목을 실제로 구현하는 방법이 바로 여기에 있습니다.
...
Redis 설치·기동은 [09-5](09-5-redis-why.md)에서 끝냈습니다. 여기서는 설치된 Redis로 바로 실습합니다.

## Pub/Sub와 폴링 — 언제 무엇을 쓸까

두 방식은 경쟁 관계가 아니라 서로 모자란 부분을 메워 줍니다.
...
**혼합 전략**: 이벤트 알림은 Pub/Sub으로 보내고, 상태 조회는 폴링으로 읽습니다.
...
09-5에서 설치한 Redis로 터미널 두 개를 열어 실습합니다.
...
팀이 키 이름을 통일해 두면 충돌과 혼선을 막을 수 있습니다. 표준 스키마는 다음과 같습니다.
...
redis-cli SET agent:서연:task "09-6 페이지 작성"
...
하트비트는 에이전트가 살아있다고 알리는 신호입니다. TTL을 함께 걸어 두면 갑자기 죽어도 자동으로 "응답 없음"으로 바뀝니다.
...
> 💡 **갱신 주기 × 8배 여유를 두는 이유** 15초마다 갱신하는데 TTL을 20초로 설정하면, 한 번이라도 갱신이 지연되면 키가 만료됩니다. 8배(120초)를 주면 갱신이 7번 연속 실패해도 키가 살아있습니다. 잠깐 처리가 늦어졌다고 잘못된 경보가 울리는 일을 막습니다.
...
기본 Pub/Sub는 오프라인 구독자에게 메시지를 전달하지 못합니다. **Redis Streams**를 사용하면 메시지가 로그처럼 쌓여 나중에도 읽을 수 있습니다.
...
> **결정 규칙**: 이벤트가 유실되면 다음 단계 작업이 시작되지 않나? → Streams. 유실돼도 폴링으로 메울 수 있나? → Pub/Sub.
...
서연이 Phase 8을 완료하고 태양에게 리뷰를 요청하는 흐름을 Redis로 구현합니다.
...
> 💡 **Streams + Pub/Sub를 동시에 쓰는 이유** Pub/Sub는 태양이 온라인이면 즉시 알립니다. Streams는 태양이 오프라인이었더라도 나중에 이벤트를 복구합니다. 둘을 함께 쓰면 빠르면서도 안정적입니다. 리뷰 큐(LPUSH)는 중복 수신 방지 역할도 합니다 — 태양이 RPOP으로 꺼내면 다른 에이전트가 같은 파일을 또 리뷰하지 않습니다.

<hr>

## 장애 감지 패턴

쭌의 5분 체크 루프에서 하트비트 만료를 감지하는 스크립트 예시입니다.
...
| 09-4 연계 | 09-4 락 패턴의 정규화·확장판이 이 절 |
| Streams vs Pub/Sub | 소실 허용 → Pub/Sub, 소실 불가 → Streams |

> **이 절의 핵심 한 줄**: Redis는 누가 살아있고 무엇을 하며 어떤 일이 있었는지를 팀 전체에 기록·전달하는 중심축입니다.
